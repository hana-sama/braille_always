...
import { MultiCellEntry, Mode } from "../data/types";
 ...
 */
export class MultiCellMatcher {
  private entries: MultiCellEntry[] = [];
   ...
   */
  setEntries(entries: MultiCellEntry[]): void {
   ...
   */
  tryMatch(dotsKey: string, mode: Mode): MultiCellMatchResult {
    this.pendingCells.push(dotsKey);
    const pendingKey = this.pendingCells.join("|");

    // Filter entries relevant to this mode
    const modeEntries = this.entries.filter(
      e => e.mode === mode || e.mode === "grade1"
    );
    ...
    const hasLongerPrefix = modeEntries.some(e =>
      e.dotsKey.startsWith(pendingKey + "|")
    );
...
export type MultiCellMatchResult =
  | { type: "matched"; entry: MultiCellEntry }
  | {
      type: "matched_with_leftover";
      entry: MultiCellEntry;
